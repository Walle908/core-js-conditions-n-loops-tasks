...
function canQueenCaptureKing(queen, king) {
  ...
  let queenX = queen.x;
  let queenY = queen.y;
  const queenDiagonalMoves = [];

  while (queenX <= 8 && queenY <= 8) {
    queenDiagonalMoves.push({ x: queenX, y: queenY });
    queenX += 1;
    queenY += 1;
  }
  ...
  queenDiagonalMoves.forEach((item) => {
    if (item.x === king.x && item.y === king.y) result = true;
  });
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    for (let j = 0; j < matrix.length / 2; j += 1) {
      const temp = result[i][j];
      result[i][j] = result[i][matrix.length - 1 - j];
      result[i][matrix.length - 1 - j] = temp;
    }
  }
...
function shuffleChar(str, iterations) {
  const repeater = str.length - 2;
  const iter = iterations - Math.floor(iterations / repeater) * repeater;
  ...
  const basic = head + tail;

  if (iter === 1) return basic;
  return shuffleChar(basic, iter - 1);
}
